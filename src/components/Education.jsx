...
export default function Education() {
  ...
  return (
    <section id="education" className="section-container relative">
      {/* Subtle section bg */}
      <div
        className="absolute inset-0 -mx-[9999px] pointer-events-none"
        style={{
          background: isDark ? 'rgba(5,11,24,0.4)' : 'rgba(241,245,249,0.45)',
          zIndex: -1,
        }}
      />
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10">
              ...
              {educationData.map((edu, i) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center flex-shrink-0">
                    <div
                      className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center text-lg sm:text-xl shadow-lg relative z-10 flex-shrink-0"
                      style={{
                        background: edu.current
                          ? `linear-gradient(135deg, ${theme.primary}, ${theme.secondary})`
                          : isDark ? '#1e293b' : '#e2e8f0',
                        border: `2px solid ${edu.current ? theme.primary : isDark ? '#334155' : '#cbd5e1'}`,
                        boxShadow: edu.current ? `0 0 18px ${theme.glow}` : 'none',
                      }}
                    >
                      {edu.emoji}
                    </div>
                    {i < educationData.length - 1 && (
                      <div
                        ...
                        style={{
                          background: `linear-gradient(to bottom, ${theme.primary}60, transparent)`,
                          minHeight: '40px',
                        }}
                      />
                    )}
                  ...
                  {/* Card */}
                  <div
                    className={`glass-card flex-1 p-4 sm:p-6 min-w-0 ${edu.current ? 'ring-1' : ''}`}
                    style={edu.current ? { '--tw-ring-color': theme.cardBorder } : {}}
                  >
                    {edu.current && (
                      <span
                        className="inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-full mb-3"
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                      <h3 className={`text-sm sm:text-lg font-bold leading-snug ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                        {edu.degree}
                      </h3>
                      <span
                        className="text-[10px] sm:text-xs font-semibold px-2 sm:px-2.5 py-1 rounded-full flex-shrink-0 self-start"
                        style={{
                    ...
                    <p className={`text-xs sm:text-sm mb-3 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                      🏛️ {edu.inst}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {edu.tags.map((t, ti) => (
                        <span
                          key={ti}
                          className={`text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg font-semibold ${
          ...
          >
            <div className="sticky top-24 glass-card p-6 sm:p-8">
              <div
              ...
              <div className={`space-y-4 text-xs sm:text-sm leading-7 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                <p>
              ...
              <div
                className="mt-6 pt-5 text-xs sm:text-sm font-medium italic"
                style={{
